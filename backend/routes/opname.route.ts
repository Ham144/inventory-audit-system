  ...
  if (!session) {
    ...
    try {
      const response = await axios.get(
        `${process.env.DATABASE_CENTER || "http://192.168.169.12:7047"}/api/v1/product/list?limit=50`,
      );
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        const response = await axios.get(
          `${process.env.DATABASE_CENTER || "http://192.168.169.12:7047"}/api/v1/product/getStock?No=${item.sku}&locationCode=${loc}`,
        );
    ...
    try {
      ...
      for (const session of activeSessions) {
        ...
        for (const item of items) {
          try {
            const response = await axios.get(
              `${process.env.DATABASE_CENTER || "http://192.168.169.12:7047"}/api/v1/product/getStock?No=${item.sku}&locationCode=${session.locationCode}`,
            );
